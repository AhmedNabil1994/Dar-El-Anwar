...
class DashboardController extends Controller
{
    ...
    public function profile()
    {
        ...
        return view('frontend.student.settings.profile', compact('data'));
    }
    ...
    public function saveInstructorInfo(Request $request)
    {
        ...
        if (Instructor::where('user_id', Auth::user()->id)->count() > 0)
        {
            ...
            if (Instructor::where('slug', getSlug(auth::user()->name))->count() > 0)
            {
                $slug = getSlug(Auth::user()->name) . '-'. rand(100000, 999999);
            } else {
                $slug = getSlug(Auth::user()->name);
            }
