    ...
    public function login(Request $request)
    {
        ...
        if(Auth::guard('admins')->attempt($credentials)) {
            ...
            if (get_option('registration_email_verification') == 1){
                $user = Auth::guard('admins')->user()->hasVerifiedEmail();
                if (!$user){
                    Auth::logout();
        ...
        else if(Auth::guard('instructors')->attempt($credentials)) {
            if (get_option('registration_email_verification') == 1){
                $user = Auth::guard('instructors')->user()->hasVerifiedEmail();
                if (!$user){
                    Auth::logout();
                    $this->showToastrMessage('error', __('Your email is not verified!'));
                    return redirect("login");
        ...
        else if(Auth::guard('parents')->attempt($credentials)) {
            if (get_option('registration_email_verification') == 1){
                $user = Auth::guard('parents')->user()->hasVerifiedEmail();
                if (!$user){
                    Auth::logout();
                    $this->showToastrMessage('error', __('Your email is not verified!'));
                    return redirect("login");
