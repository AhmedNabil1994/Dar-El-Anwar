                                ...
                                <nav aria-label="breadcrumb">
                                    <ul class="breadcrumb">
                                        <li class="breadcrumb-item"><a
                                                href="{{route('admin.dashboard')}}">{{__('لوحة التحكم')}}</a></li>
                ...
                <div class="form-horizontal__item bg-style">
                    <div class="col-md-12">
                        ...
                        <form action="{{route('role.store')}}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="custom-form-group mb-3 row">
                                <label for="name"
                                       class="col-lg-3 text-lg-right text-black"> {{__('website.name')}} </label>
                                <div class="col-lg-9">
                                    <input type="text" name="name" id="name" value="{{old('name')}}"
                                           class="form-control flat-input" placeholder="{{__('website.name')}}">
                                    @if ($errors->has('name'))
                            ...
                            <div class="custom-form-group mb-3 row">
                                <label for="name"
                                       class="col-lg-3 text-lg-right text-black"> {{__('اختر الصلاحية')}} </label>
                                <div class="col-lg-9">
...
@push('script')
    <script>
        $('.collect_permissions').on('click', function () {
        ...
        $('.collect_permissions').each(function () {
            var name = $(this).data('name');
            var parent = $(this);
            checkbox = $(`.${name}`)
