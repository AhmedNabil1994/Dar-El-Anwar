...
@section('content')
    <div class="card">
        <div class="card-header">
            <h6 class="mb-0">{{ __('الرسائل') }} {{ $chatThread->instructor->name }}</h6>
        </div>
        <div class="card-body">
...
@push('script')
    <script>
        ...
        function refreshChat(){
            $.post('{{ route('student.chats.refresh') }}', {_token: '{{ csrf_token() }}', id: $('input[name=chat_thread_id]').val(), lastMessageId: lastMessageId}, function(data){
        ...
        function send_reply(e){
            e.preventDefault();
            $.post('{{ route('student.chats.reply') }}', {
                _token: '{{ csrf_token() }}',
