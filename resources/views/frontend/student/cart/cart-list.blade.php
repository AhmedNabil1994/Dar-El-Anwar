                                ...
                                <div class="page-banner-content text-center">
                                    ...
                                    <nav aria-label="breadcrumb">
                                        <ol class="breadcrumb justify-content-center">
                                            ...
                                            <li class="breadcrumb-item font-14 active" aria-current="page">{{ __('Cart') }}</li>
                                        </ol>
                                    </nav>
                                    <!-- Breadcrumb End-->
                                </div>
                    ...
                    <div class="col-lg-9">
                        <div class="cart-page-left-part bg-white">
                            <div class="cart-page-title d-flex justify-content-between align-items-center mb-3">
                                ...
                                <a href="{{ route('courses') }}" class="text-decoration-underline font-14 font-medium">{{__('Continue Buying')}}</a>
                            </div>
                                                    ...
                                                    <div class="course-img-wrap flex-shrink-0 overflow-hidden">
                                                        ...
                                                        @if($cart->course_id)
                                                            <a href="{{ route('course-details', @$cart->course->slug) }}">
                                                                <img src="{{ getImageFile(@$cart->course->image_path) }}" alt="course" class="img-fluid"></a>
                                                        @elseif($cart->bundle_id)
                                                            <a href="{{ route('bundle-details', [@$cart->bundle->uuid, @$cart->bundle->slug]) }}">
                                                                <img src="{{ getImageFile(@$cart->bundle->image) }}" alt="bundle course" class="img-fluid"></a>
                                                        @elseif($cart->consultation_slot_id)
                                                            <a href="{{ route('instructorDetails', [@$cart->consultationSlot->user->id, Str::slug(@$cart->consultationSlot->user->instructor->full_name)]) }}">
                                                                <img src="{{ getImageFile(@$cart->consultationSlot->user->image) }}" alt="Consultator" class="img-fluid"></a>
                                                        @endif
                                                    </div>
                                                    <div class="card-body flex-grow-1">
                                                        <h5 class="card-title course-title">
                                                            @if($cart->course_id)
                                                                <a href="{{ route('course-details', @$cart->course->slug) }}">{{ @$cart->course->title }}</a>
                                                            @elseif($cart->bundle_id)
                                                                <a href="{{ route('bundle-details', [@$cart->bundle->uuid, @$cart->bundle->slug]) }}">{{ @$cart->bundle->name }}</a>
                                                            @elseif($cart->consultation_slot_id)
                                                                <a href="{{ route('instructorDetails', [@$cart->consultationSlot->user->id, Str::slug(@$cart->consultationSlot->user->instructor->full_name)]) }}">
                                                                    {{ @$cart->consultationSlot->user->instructor->full_name }}
                                                                </a>
                                                            @endif
                                                        </h5>
                                                        <p class="card-text instructor-name-certificate font-medium">
                                                            ...
                                                            @elseif($cart->bundle_id)
                                                                ...
                                                                @if(get_instructor_ranking_level(@$cart->course->bundle->user_id))
                                                                    | {{ get_instructor_ranking_level(@$cart->course->bundle->user_id) }}
                                                                @endif
                                                            @elseif($cart->consultation_slot_id)
                                                                {{ @$cart->consultationSlot->user->instructor->professional_title }}
                                                                @if(get_instructor_ranking_level(@$cart->consultationSlot->user->id))
                                                                    | {{ get_instructor_ranking_level(@$cart->consultationSlot->user->id) }}
                                                                @endif
                                                            @endif
                                                        </p>
                                                        @if($cart->course_id)
                                                    ...
                                                    <form class="row row-cols-lg-auto g-3 align-items-center">
                                                        <div class="col-12">
                                                            <div class="input-group">
                                                                <input type="text" class="form-control font-14 coupon-code-{{$cart->id}}" placeholder="{{__('Coupon Code')}}">
                                                            </div>
                                                        </div>
                                                        <div class="col-12">
                                                            <button type="button" class="theme-btn btn-violet-transparent form-control font-14 apply-coupon-code"
                                                                    data-id="{{$cart->id}}" data-route="{{ route('student.applyCoupon') }}">{{__('Apply')}}</button>
                                                        </div>
                                                    </form>
                                                </div>
                                                <!-- Apply Voucher End -->
                                            ...
                                            <td class="wishlist-price font-15 color-heading">
                                                <div class="show-price-number">
                                                    @if(get_currency_placement() != 'after')
                                                        <span class="price-symbol"> {{get_number_format(@$cart->price, 2)}}</span><span>{{ get_currency_symbol() }}</span>
                                                    @else
                                                        <span class="price-symbol">{{ get_currency_symbol() }}</span><span>{{get_number_format(@$cart->price, 2)}}</span>
                                                    @endif
                                                </div>
                                            </td>
                            ...
                            <form method="POST" action="{{route('student.goToCheckout')}}">
                                ...
                                <div
                                    ...
                                    <span>
                                        @if(get_currency_placement() == 'after')
                                            <span
                                                class="grand-total">{{ get_number_format($carts->sum('price') + get_platform_charge($carts->sum('price')))  }}</span> {{ get_currency_symbol() }}
                                        @else
                                            {{ get_currency_symbol() }} <span
                                                class="grand-total">{{ get_number_format($carts->sum('price') + get_platform_charge($carts->sum('price')))  }}</span>
                                        @endif
                                ...
                                <div class="cart-check-terms-condition d-flex my-3">
                                    ...
                                    <div class="flex-grow-1 ms-1">
                                        By placing your order, you agree with our company <a href="{{ route('privacy-policy') }}">privacy policy</a> and <a
                                ...
                                <div class="order-summary-btns">
                                    ...
                                    @if(@Auth::user()->is_instructor())
                                        <input type="submit" class="theme-btn btn-orange w-100 justify-content-center mt-3" value="Pay from  wallet"
                                               name="pay_from_lmszai_wallet">
                                    @endif
