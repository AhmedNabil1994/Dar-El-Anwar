            ...
            <div class="row">
                <div class="col-lg-3">
                    @if(Auth::guard('instructors')->check())
                        @foreach(\App\Models\Student::whereStatus(1)->get() as $student)
                            <div class="row">

                                <div class="col-md-9 p-3 m-3 border border-dark">

                                    <img width="25" height="25" src="{{asset($student->getImg())}}"
                                          class="bx-circle">

                                    <a href="{{route('instructor.chats.create',['student' => $student->id])}}">{{$student->name}}</a>
                                </div>
                            </div>
                        @endforeach
                    @endif
                            ...
                            <table class="table aiz-table mb-0">
                                ...
                                @foreach($chat_threads as $key => $chat_thread)
                                    <tr>
                                        ...
                                        <td class="text-right">
                                            <a href="{{ route('instructor.chats.show', $chat_thread->id) }}"
